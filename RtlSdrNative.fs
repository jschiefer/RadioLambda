...
[<DllImport(PlatformLibrary, CallingConvention=PlatformCallingConvention)>]
extern nativeint rtlsdr_get_device_name(uint32 index)
...
// I wasted a bunch of time right here - turns out that F# delegates come in two 
// flavors: 
// - delegate with tuple arguments, e.g. type Delegate1 = delegate of (int * int) -> int
// - delegate with curried arguments, e.g. type Delegate2 = delegate of int * int -> int
//
// The parentheses make all the difference! If you get this wrong, your delegate will 
// still be invoked, but the arguments will be cobbled and the call stack will be 
// trashed.
// More information at 
